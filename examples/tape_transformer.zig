/// TapeTransformerStack benchmark — same task as big_transformer but through the autograd tape.
/// Verifies identical GPU performance: all layers still run in one Metal command buffer.
///
/// Run: zig build tape-transformer
...
const Stack = zaneml.TapeTransformerStack(BACKEND, D_MODEL, N_HEADS, D_FF, MAX_SEQ, N_LAYERS, .{
    .norm           = .layer_norm,
    .ffn_activation = .gelu,
    .causal         = false,
});
...
pub fn main() !void {
    ...
    // Same synthetic task as big_transformer: learn output = -input
    ...
    for (0..N_SAMPLES) |s| {
        for (0..INPUT_SIZE) |i| {
            inputs[s][i]  = rand.float(f32) * 2.0 - 1.0;
    ...
    std.debug.print("TapeTransformerStack: d_model={d}, n_heads={d}, d_ff={d}, layers={d}, seq={d}\n", .{
        D_MODEL, N_HEADS, D_FF, N_LAYERS, SEQ_LEN,
    });
    std.debug.print("Training: {d} samples x {d} epochs  (Metal batch path)\n\n", .{ N_SAMPLES, EPOCHS });
